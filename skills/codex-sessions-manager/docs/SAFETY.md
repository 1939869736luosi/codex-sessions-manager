...
Codex roots may also contain `logs_N.sqlite`, `memories_N.sqlite`, remote-control state, external agent imports, and compressed rollout files (`.jsonl.zst`). A confirmed permanent delete removes only exact `logs.thread_id` rows. Trash retains those rows so restore remains complete; final purge removes them unless the same ID is live again. This is logical row deletion, not a promise that WAL, free pages, backups, or filesystem snapshots contain no old bytes. Compressed rollout files are treated as session files and stored in trash/backup bundles as binary-safe data. Treat memory DB rows as official derived state, not disposable residue: memory and Phase 2 outputs remain read-only and are never modified by session cleanup.

Dedicated-log mutation requires a recognized `logs` table with `thread_id` and a stable primary-key `id`. Direct deletion records at most 10,000 rows and 32 MiB of affected log data for rollback; larger targets fail before any mutation. Purge records only stable row keys, with limits of 100,000 keys, 64 KiB per key component, and 16 MiB for the encoded key payload. Count, size, and key collection use one SQLite snapshot. Preparation and commit compare the fixed key set, so concurrent inserts produce `STALE_PLAN` instead of being swept into the delete. Use smaller explicit session batches when a safety limit is reached.

When reporting `verify` results, say that the result applies to the cleanup surfaces this release supports. Do not claim physical byte erasure or unconditional zero-orphan coverage: memory mutation, unowned logs, WAL/free pages, backups, and filesystem snapshots remain outside scope.
...
- no shell snapshots
- no supported SQLite cleanup rows, including exact thread-linked log rows for permanent delete
